...
```
index.html              → Monolito principal con inventoryApp() en L2184
src/
...
> ⚠️ **CRÍTICO:** La lógica de negocio vive en `index.html` dentro de `inventoryApp()` (línea ~2184). NO existe `src/app.js` - todo el estado reactivo Alpine.js está en el HTML principal. El archivo tiene ~5600 líneas.
...
|---------|-----------|-----------|
| `calculateRowLogic(item)` | index.html:L3949 | Cálculo compra/stock (UXB, SS, stockMin/Max, seasonalMult, ventaPerdida) |
| `getColumns()` | index.html:L5080 | Define columnas visibles por tab |
| `getCellClass(col, row)` | index.html | Estilos condicionales por celda (colores, badges) |
| `generateSnapshot()` | data-processor.js:L35 | Genera snapshots de VTAR por SKU para historial |
| `buildLookups()` | data-processor.js:L56 | Mapea hojas auxiliares a objetos lookup |
| `validateSheet()` | data-processor.js:L22 | Valida columnas requeridas en hojas Excel |
...
|-------|---------|--------|
| Agregar columna tabla | index.html | `getColumns()` (~L5080) |
| Cambiar cálculo compra/stock | index.html | `calculateRowLogic()` (~L3949) |
...
- **Columna no aparece:** revisar que esté en `getColumns()` para el tab correcto (~L5080)
...
```bash
npm test              # Vitest watch mode - ejecuta tests/utils.test.js
npm run test:ui       # Interfaz visual de tests en navegador
```

- Tests cubren utilidades en `src/utils.js` (parseNumber, formatMoney, etc.)
- Al agregar funciones a utils.js, agregar tests correspondientes
- No hay tests E2E actualmente - validación manual con archivos Excel
