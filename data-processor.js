...
export function generateSnapshot(rawGrafana) {
    try {
        ...
        const iVtar = findColumnIndex(hG, ['VTASQ60', 'VTAR']);
...
export function processGrafanaData(rawGrafana) {
    ...
    validateSheet(hG, ['SKU', 'Stock']);

    // Índices de columnas
    const indices = {
        sku: findColumnIndex(hG, ['SKU']),
        dep: findColumnIndex(hG, ['Deposito']),
        vtar: findColumnIndex(hG, ['VTASQ60', 'VTAR']),
        vpd: findColumnIndex(hG, ['VPD_Cpra']),
        vtacq60: findColumnIndex(hG, ['VTACQ60']),
        vtasq7: findColumnIndex(hG, ['VTASQ7']),
        vtasq15: findColumnIndex(hG, ['VTASQ15']),
        vtasq30: findColumnIndex(hG, ['VTASQ30']),
        vtasq45: findColumnIndex(hG, ['VTASQ45']),
    ...
    rawGrafana.slice(1).forEach(r => {
        ...
        // DEBUG: Log de valores crudos antes de parsear
        logger.info('[DEBUG] Valores crudos fila', {
            sku: r[indices.sku],
            vtar: r[indices.vtar],
            stock: r[indices.stock],
            costo: r[indices.costo],
            lt: r[indices.lt],
            vpd: r[indices.vpd],
            uxb: r[indices.uxb]
        });
        ...
        if (histIndices.length > 0 && data[sku].histDaily.length === 0) {
            ...
            // Calcular promedios por períodos (fallback si no vienen del Excel)
            const calcAvg = (days) => {
        ...
        // Acumular nuevas columnas VTASQ del Excel
        if (indices.vtacq60 > -1) data[sku].vtacq60 += parseNumber(r[indices.vtacq60]);
        if (indices.vtasq7 > -1) data[sku].vtasq7 += parseNumber(r[indices.vtasq7]);
        if (indices.vtasq15 > -1) data[sku].vtasq15 += parseNumber(r[indices.vtasq15]);
        if (indices.vtasq30 > -1) data[sku].vtasq30 += parseNumber(r[indices.vtasq30]);
        if (indices.vtasq45 > -1) data[sku].vtasq45 += parseNumber(r[indices.vtasq45]);
